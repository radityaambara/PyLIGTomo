...
class VelocityGrid:
    ...
    def vel_vectorize(self,x,y,z):
        ix = ((x - self.xmin) / self.deltn).astype(int)
        iy = ((y - self.ymin) / self.deltn).astype(int)
        iz = ((z - self.zmin) / self.deltn).astype(int)
        ix1 = ix + 1
        iy1 = iy + 1
        iz1 = iz + 1
    ...
    def tt(self,path):
        ...
        #vectorize tt calculation
        path_mirror=path[1:]
        distance_path=np.linalg.norm(path[:-1]-path_mirror,axis=1)
        velo=self.vel_vectorize(path[:, 0], path[:, 1], path[:, 2])
        slow=1/velo
        s1 = slow[:-1]
        s2 = slow[1:]
...
#               R = rcur
    ...
    def doublepath(self,path):
        diffs=path[1:]-path[:-1]
        dis=np.linalg.norm(diffs,axis=1)
        if np.any(dis < 2*self.delt):
            return np.copy(path)
        midpoints = path[:-1]+(diffs/2)
        pathnew=np.empty((path.shape[0]+midpoints.shape[0],path.shape[1]),dtype=path.dtype)
        pathnew[0::2]=path
        pathnew[1::2]=midpoints
        return pathnew
    ...
    """handling error for forward modeling when using multiprocessing"""
    def safe_crkernel(self,path):
        try:
            return self.crkernel(path)
    ...
    def safe_ttime_only(self,path):
        try:
            return self.ttime_only(path)
        except Exception as e:
            return {
                "error": str(e),
                "type": type(e).__name__,
...
class is_addnode:
    def __init__(self,hit_count,tres,node,deltn,interpP,interpS,nu_cpu=-1):
    ...
    def add_simultanius(self,sim):
        ...
        distance, index = node_tree.query(add_node, k=1, workers=self.nu_cpu)
