...
def run_invers(modvel, modvel_outer, source_list, receiver_list, phase_list, delt, deltn, xfac, iter1, iter2, tmin,
    ...
    #load data
    source_list.columns=['id','easting','northing','depth']
    ...
    for i in range(0, len(paths_P)):
        try:
            ...
            logger.info('error = '+str(f1[i].exception()))
            continue
    ...
    for iter in range(0, iteration_number):
        ...
        inv_matrix_P=np.matmul(np.vstack((vp_kernel_inv,vp_kernel_zeros,smooth_damp,zeros_smooth)),weight_inv)
        inv_matrix_S=np.matmul(np.vstack((vs_kernel_zeros,vs_kernel_inv,zeros_smooth,smooth_damp)),weight_inv)
